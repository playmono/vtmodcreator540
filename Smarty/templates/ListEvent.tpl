...
<tr>
	...
	{if count($EVENTS)}
		...
		<tr>
			<td class="big">Module Events</td>
			<td class="small" align=right>
				<input type='button' class='create' value='Create New' onclick="location.href='index.php?action=EditEvent&moduleid={$MODULE->id}'">
			</td>
		</tr>
		</table>
		...
		<tr>
			...
			<td class="colHeader">Class</td>
			<td class="colHeader">File</td>
			<td class="colHeader">Actions</td>
		</tr>
		{foreach item=EVENT from=$EVENTS}
			<tr>
				<td class="cellText">{$EVENT->id}</td>
				<td class="cellText">{$EVENT->eventname}</td>
				<td class="cellText">{$EVENT->classname}</td>
				<td class="cellText">{$EVENT->filename}</td>
				<td class="cellText">
					<a href='index.php?action=EditEvent&moduleid={$MODULE->id}&id={$EVENT->id}'>Edit</a> |
					<a href='index.php?action=SaveEvent&moduleid={$MODULE->id}&id={$EVENT->id}&mode=delete'>Delete</a>
				</td>
			</tr>
		{/foreach}
		</table>
	{else}
		...
		<tr>
			<td>
			<input type='button' class='create' value='Create New Event' onclick="location.href='index.php?action=EditEvent&moduleid={$MODULE->id}'">
			</td>
		</tr>
		</table>
	{/if}
